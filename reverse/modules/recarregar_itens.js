/* TB-WM u=179 e=a07d5b4cfa x=f2e5047b9289 t=1786720275 s=7e5c5419de4417b7 */
(function(){try{window.__TIBIABOT_WM__={u:179,t:1786720275,x:"f2e5047b9289",s:"7e5c5419de4417b7"};}catch(e){}})();
...
// MÃ³dulo Baiak Idle (MAIN): Recarregar itens (silver token)
// - Monitora backpack/inv por itens vazios recarregÃ¡veis
// - Clique realista no item â botÃ£o com "recarregar" â confirma se aparecer
// - Custo: silver token (object 22516)

(function () {
  const POLL_MS = 1600;
  const BETWEEN_MS = 500;
  const MENU_WAIT_MS = 4000;
  const CONFIRM_WAIT_MS = 2500;
  const SETTLE_MS = 1000;
  const SKIP_MS = 20000;
  const STATUS_UI_MS = 1000;
  const SILVER_TOKEN_ID = 22516;
  const SILVER_TOKEN_NAME = 'silver token';

  /** Itens recarregÃ¡veis com silver token quando sem charges/wear. */
  const RECHARGE_BY_NAME = new Set([
    'spiritthorn ring',
    'might ring',
    'stone skin amulet',
    'ssa',
  ...
  class BaiakIdleRecarregarItensModule {
    constructor() {
      this._running = false;
      this._busy = false;
    ...
    _log(msg, extra) {
      try {
        if (window.__BAIAKIDLE_DEBUG) {
          console.log('[BaiakIdle Recarregar]', msg, extra || '');
        }
      } catch (_) {}
    }
    ...
    async _waitFor(fn, timeoutMs, intervalMs = 100) {
        ...
        window.postMessage(
          {
            ...
            payload: {
              botId: 'baiak_idle',
              botLabel: 'Baiak-Idle',
              moduleId: 'recarregar_itens',
              moduleLabel: 'Recarregar',
              status,
              remainingMs: 0,
    ...
    _startStatusTicker() {
      ...
      const tick = () => {
        ...
        if (this._busy) {
          this._emitStatus('recharging', this._lastAction || 'â¦');
          return;
        }
        const tokens = this._countSilverTokens();
        if (tokens <= 0) {
          this._emitStatus('no_tokens', 'sem silver token');
          return;
        }
        const n = this._listCandidates().length;
        if (n > 0) {
          this._emitStatus('ready', tokens + ' tok Â· ' + n + ' vazio(s)');
        } else {
          this._emitStatus('idle', tokens + ' tok Â· ok');
        }
      };
      tick();
      this._statusTimer = setInterval(tick, STATUS_UI_MS);
    }
    ...
    /**
     * Clique com sequÃªncia pointer/mouse + coordenadas
     * (cÃ©lulas do inventÃ¡rio costumam ignorar sÃ³ el.click()).
     */
    _humanClick(el, label) {
      if (!el) return false;
      const S = this._sel();
      try {
      ...
      const target =
        el.querySelector?.('img') && el.classList?.contains?.('cell')
          ? el
          : el;
      try {
      ...
      try {
        // PointerEvent quando existir (jogos modernos escutam pointer*)
        if (typeof PointerEvent === 'function') {
          ...
          target.dispatchEvent(new PointerEvent('pointerup', { ...pOpts, buttons: 0, pressure: 0 }));
        }
      } catch (_) {}
      ...
      this._log('click', { label, x, y, tag: target?.tagName });
      return true;
    }

    _clickSafe(el, label) {
      if (!el) return false;
      // NÃ£o bloquear por isClickable em cÃ©lulas de inventÃ¡rio â vÃ¡rias ficam
      // technicalmente ok mas o helper do seletor pode falhar com containers.
      try {
        const S = this._sel();
        if (S && typeof S.isClickable === 'function') {
          const isCell = !!el.closest?.('.cell, .invgrid');
          if (!isCell && !S.isClickable(el)) return false;
        }
      } catch (_) {}
      return this._humanClick(el, label);
    }

    /** Grids de inventÃ¡rio onde o anel vazio / tokens podem estar. */
    _itemGrids() {
      const S = this._sel();
      const out = [];
      const push = (el) => {
      ...
      if (S?.ITEMS?.BACKPACK_GRID) {
        push(S.findElement(S.ITEMS.BACKPACK_GRID, 'BACKPACK_GRID'));
      }
      if (S?.ITEMS?.INV_GRID) {
        push(S.findElement(S.ITEMS.INV_GRID, 'INV_GRID'));
      }
      push(document.querySelector('#backpack-grid'));
      push(document.querySelector('#inv-grid'));
      // slots de equipamento (anel etc.)
      document
        .querySelectorAll(
          '#eq-grid, #equip-grid, .eqgrid, .equip-grid, #player-equip, .eq-slots, #eq'
        )
        .forEach((el) => push(el));
      return out;
    }

    _isSilverTokenImg(img) {
      if (!img) return false;
      const alt = String(img.getAttribute('alt') || '').trim().toLowerCase();
      const src = String(img.getAttribute('src') || '');
      return (
        alt === SILVER_TOKEN_NAME ||
        src.includes('/object/' + SILVER_TOKEN_ID) ||
        src.includes('object/' + SILVER_TOKEN_ID + '.') ||
        /\/22516(\.png|\?)/.test(src)
      );
    }
    ...
    _countSilverTokens() {
      let total = 0;
      const roots = this._itemGrids();
      // fallback: documento inteiro (token fora do grid esperado)
      if (!roots.length) roots.push(document.body || document.documentElement);

      const seen = new Set();
      for (const root of roots) {
        if (!root?.querySelectorAll) continue;
        const cells = root.querySelectorAll('.cell, .mat');
        for (const cell of cells) {
          if (seen.has(cell)) continue;
          seen.add(cell);
          const img = cell.querySelector?.('img');
          if (!this._isSilverTokenImg(img)) continue;

          const tip = String(
            cell.getAttribute('data-tip') ||
              cell.getAttribute('title') ||
              cell.getAttribute('data-tiphtml') ||
              ''
          );
          const mQty = tip.match(/(\d+)\s*x\s*silver token/i);
          if (mQty) {
            total += parseInt(mQty[1], 10) || 0;
            continue;
          }
          const qtyEl = cell.querySelector('.qty, span.qty');
          const q = parseInt(String(qtyEl?.textContent || '').replace(/\D/g, ''), 10);
    ...
    _cellName(cell) {
      const S = this._sel();
      const info = typeof S?.parseCmpItem === 'function' ? S.parseCmpItem(cell) : null;
      if (info?.name) return String(info.name).trim().toLowerCase();
    ...
    _cellUid(cell) {
      const S = this._sel();
      const info = typeof S?.parseCmpItem === 'function' ? S.parseCmpItem(cell) : null;
      if (info?.uid != null) return info.uid;
      if (info?.hash) return info.hash;
      return this._cellName(cell) || null;
    }
    ...
    _readCharges(cell) {
      const S = this._sel();
      const info = typeof S?.parseCmpItem === 'function' ? S.parseCmpItem(cell) : null;
      if (info && typeof info.chg === 'number' && Number.isFinite(info.chg)) {
        return info.chg | 0;
      }
      const wear = cell.querySelector?.('.wear, span.wear');
      if (wear) {
        const n = parseInt(String(wear.textContent || '').replace(/\D/g, ''), 10);
        if (Number.isFinite(n)) return n;
      }
      const tip = this._tipHtml(cell);
      // SÃ³ casa linha explÃ­cita de Charges / Cargas (evita falso positivo em "Lv.1")
      const m =
        tip.match(/Charges<\/span>\s*<span[^>]*>\s*(\d+)/i) ||
        tip.match(/>\s*Charges\s*<\/span>\s*<span[^>]*>\s*(\d+)/i) ||
        tip.match(/(?:Charges|Cargas)\s*[:ï¼]?\s*(\d+)/i);
      if (m) {
        const n = parseInt(m[1], 10);
        if (Number.isFinite(n)) return n;
      }
    ...
    /** spiritthorn vazio costuma vir com sprite data: e sem charges/wear. */
    _looksEmptySprite(cell) {
      const img = cell.querySelector?.('img');
      if (!img) return false;
      const src = String(img.getAttribute('src') || '');
    ...
    _needsRecharge(cell) {
      if (!cell || cell.classList?.contains('mat')) return false;
      const img = cell.querySelector?.('img');
      if (!img) return false;
      if (this._isSilverTokenImg(img)) return false;

      const name = this._cellName(cell);
      if (!name) return false;

      const isKnown = RECHARGE_BY_NAME.has(name);
      const charges = this._readCharges(cell);

      if (charges != null) {
        return charges <= 0 && (isKnown || charges === 0);
      }

      // Sem charges no DOM: sÃ³ tenta se for item da lista (anel vazio / sprite apagado)
      if (!isKnown) return false;

      // Se o sprite estÃ¡ âapagadoâ (data URL) ou nÃ£o hÃ¡ badge .wear â vazio
      if (this._looksEmptySprite(cell)) return true;
      if (!cell.querySelector?.('.wear, span.wear')) return true;
    ...
    _isSkipped(uid) {
      ...
      const until = this._skipUntil.get(uid);
      if (!until) return false;
      if (Date.now() >= until) {
        this._skipUntil.delete(uid);
        return false;
      }
      return true;
    }

    _skip(uid, ms = SKIP_MS) {
      if (uid == null || uid === '') return;
      this._skipUntil.set(uid, Date.now() + ms);
    }
    ...
    _listCandidates() {
      const out = [];
      const seen = new Set();
      for (const grid of this._itemGrids()) {
        if (!grid?.querySelectorAll) continue;
        const cells = grid.querySelectorAll('.cell[data-cmpitem], .cell');
        for (const cell of cells) {
          if (seen.has(cell)) continue;
          seen.add(cell);
          if (cell.classList.contains('mat')) continue;
          if (!this._needsRecharge(cell)) continue;
          const uid = this._cellUid(cell);
          if (this._isSkipped(uid)) continue;
          out.push(cell);
        }
      }
      return out;
    ...
    _findControlByTextIncludes(want, root) {
      const needle = String(want || '')
        .trim()
        .toLowerCase();
      ...
      const nodes = scope.querySelectorAll(
        'button, a[href], [role="button"], .menu-item, .im-menuitem, .ctx-item, .item-opt, .opt, li, div[class*="menu"] span, div[class*="menu"] div'
      );
      let fallback = null;
      for (const el of nodes) {
        if (el.closest?.('#tibia-bot-root, #tibia-bot-overlay, #tb-game-panel, [id*="tibia-bot"]')) {
          continue;
        }
        const text = this._visibleText(el).toLowerCase();
      ...
      if (typeof this._sel()?.findButtonByText === 'function') {
        const btn = this._sel().findButtonByText(want, { includes: true, root: scope });
        if (btn) return btn;
      }
      return null;
    }
    ...
    _findRecarregarBtn() {
      return this._findControlByTextIncludes('recarregar', document);
    }

    _findConfirmBtn() {
      const S = this._sel();
      if (S?.AUTO_SELL?.CONFIRM_YES) {
        const byId = S.findElement(S.AUTO_SELL.CONFIRM_YES, 'CONFIRM_YES');
        if (byId) {
          const t = this._visibleText(byId).toLowerCase();
          if (
            !t.includes('liberar') &&
            !t.includes('cancel') &&
            !t.includes('nÃ£o') &&
            ...
            try {
              if (!S.isClickable || S.isClickable(byId)) return byId;
            } catch (_) {
              return byId;
            }
          }
      ...
      const labels = ['confirmar', 'confirm', 'sim', 'yes', 'ok'];
      for (const lab of labels) {
        const el = this._findControlByTextIncludes(lab, document);
        if (!el) continue;
        const t = this._visibleText(el).toLowerCase();
        if (t.includes('cancel') || t.includes('nÃ£o') || t.includes('nao')) continue;
        if (t.includes('liberar')) continue;
        if (t.includes('recarregar')) continue;
        return el;
      }
      return null;
    }
    ...
    async _rechargeOne(cell) {
      const name = this._cellName(cell) || 'item';
      const uid = this._cellUid(cell);
      this._lastAction = name;
      this._log('recarregar candidato', { name, uid });

      // 1â2 cliques no item (menu de contexto do inventÃ¡rio)
      if (!this._clickSafe(cell, 'item ' + name)) {
        this._skip(uid, 8000);
        return false;
      }
      await this._sleep(280);

      let recBtn = this._findRecarregarBtn();
      if (!recBtn) {
        // 2Âº clique se o 1Âº nÃ£o abriu
        this._clickSafe(cell, 'item2 ' + name);
        recBtn = await this._waitFor(() => this._findRecarregarBtn(), MENU_WAIT_MS);
      } else {
        // menu jÃ¡ pode ter aberto â ainda assim espera um pouco se sumiu
        recBtn =
          recBtn ||
          (await this._waitFor(() => this._findRecarregarBtn(), MENU_WAIT_MS));
      }

      if (!recBtn) {
        this._log('sem botÃ£o recarregar', name);
        this._skip(uid, 12000);
        await this._dismissMenus();
        return false;
      }

      if (!this._clickSafe(recBtn, 'recarregar')) {
        this._skip(uid, 10000);
        await this._dismissMenus();
        return false;
      }

      // ConfirmaÃ§Ã£o opcional
      const confirm = await this._waitFor(() => this._findConfirmBtn(), CONFIRM_WAIT_MS);
      if (confirm) {
        this._clickSafe(confirm, 'confirm ' + this._visibleText(confirm));
      }

      await this._sleep(SETTLE_MS);
      await this._dismissMenus();
      // sucesso: limpa skip se existir
      if (uid != null) this._skipUntil.delete(uid);
      return true;
    }
    ...
    async _tick() {
      ...
      const candidates = this._listCandidates();
      if (!candidates.length) {
        this._log('sem candidatos', { tokens });
        return;
      }
      ...
      try {
        ...
        await this._rechargeOne(cell);
        await this._sleep(BETWEEN_MS);
      } catch (err) {
        this._log('erro tick', err);
      } finally {
        this._busy = false;
        this._lastAction = '';
      }
    ...
    start() {
      ...
      this._busy = false;
      this._lastStatusKey = '';
      this._skipUntil.clear();
      this._log('start');
      this._startStatusTicker();
      this._scheduleCheck();

      this._observer = new MutationObserver(() => this._scheduleCheck());
      ...
      if (root) {
        this._observer.observe(root, {
          ...
          subtree: true,
          attributes: true,
          attributeFilter: ['data-cmpitem', 'data-tiphtml', 'class', 'style', 'src']
        });
      }
  ...
  window.BaiakIdleRecarregarItensModule = BaiakIdleRecarregarItensModule;

  try {
    const prev = window.__baiakIdleRecarregarItens;
    const wasRunning = !!prev?.isRunning?.();
    try {
      prev?.stop?.();
    } catch (_) {}
    window.__baiakIdleRecarregarItens = new BaiakIdleRecarregarItensModule();
    if (wasRunning || window.__BAIAKIDLE_AUTO_START_RECARREGAR_ITENS__) {
      window.__baiakIdleRecarregarItens.start();
    }
  } catch (err) {
    console.error('[BaiakIdle Recarregar] bootstrap', err);
  }
})();
