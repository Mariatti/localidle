/* TB-WM u=179 e=a07d5b4cfa x=f2e5047b9289 t=1786720273 s=811524547c0a294e */
(function(){try{window.__TIBIABOT_WM__={u:179,t:1786720273,x:"f2e5047b9289",s:"811524547c0a294e"};}catch(e){}})();
...
// MÃ³dulo Baiak Idle (MAIN): Gold/h real
// - LÃª #an-balance (Balance = loot â supplies) e #an-session
// - PreferÃªncia: Balance Ã· tempo da Session Ã 3600 (mÃ©dia da sessÃ£o)
// - Fallback: delta Balance na janela recente (se Session indisponÃ­vel)
...
(function () {
    ...
    /** Tempo mÃ­nimo de sessÃ£o para confiar em Balance Ã· Session. */
    ...
    class BaiakIdleGoldHoraModule {
        ...
        /** Texto bruto do jogo (#an-session), ex.: "01:00:12". */
        _readSessionText() {
            const el = this._readSessionEl();
            if (!el) return '';
            return String(el.textContent || '').trim();
        }
        ...
        _readSessionSec() {
            try {
                const text = this._readSessionText();
        ...
        /** Monta texto do overlay: "11.160/h". */
        _statusText(gphLabel) {
            return String(gphLabel || 'â¦');
        }
        ...
        _resetHistory(reason) {
            if (this._samples.length || this._rateHistory.length) {
                this._log('HistÃ³rico resetado', reason || '');
            }
            this._samples = [];
            this._rateHistory = [];
            this._currentGph = null;
        ...
        /**
         * Gold/h pela mÃ©dia da sessÃ£o: Balance Ã· Session Ã 3600.
         * Ex.: 10.230 em 55 min â ~11.160/h (nÃ£o a janela dos Ãºltimos 5 min).
         */
        _computeGphFromSession(gold, sessionSec) {
            if (gold == null || sessionSec == null) return null;
            if (sessionSec < MIN_SESSION_SEC) return null;
            return (gold / sessionSec) * 3600;
        }

        /** Fallback: ritmo recente (delta Balance na janela). */
        _computeGphFromWindow() {
        ...
        _computeGph(gold, sessionSec) {
            const fromSession = this._computeGphFromSession(gold, sessionSec);
            if (fromSession != null) return fromSession;
            return this._computeGphFromWindow();
        }
        ...
        _tick() {
            ...
            try {
                ...
                if (gold == null) {
                    this._emitStatus('waiting', this._statusText('â¦'));
                    return;
                }
                ...
                const gph = this._computeGph(gold, sessionSec);
                if (gph == null) {
                    this._currentGph = null;
                    this._emitStatus('warming', this._statusText('â¦'));
                    return;
                }
                ...
                this._emitStatus(
                    'gph',
                    this._statusText(this._formatGold(gph) + '/h')
                );
            } catch (err) {
        ...
        start() {
            ...
            this._log(`MÃ³dulo iniciado (amostra a cada ${POLL_MS / 1000}s)`);
            this._tick();
