/* TB-WM u=179 e=a07d5b4cfa x=f2e5047b9289 t=1786720276 s=e23e24721d8cf0bb */
(function(){try{window.__TIBIABOT_WM__={u:179,t:1786720276,x:"f2e5047b9289",s:"e23e24721d8cf0bb"};}catch(e){}})();
...
// MÃ³dulo Baiak Idle (MAIN): Coletar recompensa (Daily Reward)
// - Detecta #tab-daily com .daily-badge (disponÃ­vel)
// - Clica na aba â #daily-claim-btn â #daily-modal-close
// - Poll leve; apÃ³s coletar aplica cooldown longo (diÃ¡rio ~24h)

(function () {
  const POLL_MS = 12000;
  const STATUS_UI_MS = 1500;
  const MODAL_WAIT_MS = 6000;
  const SETTLE_MS = 800;
  const CLAIM_COOLDOWN_MS = 6 * 60 * 60 * 1000; // 6h (evita spam apÃ³s claim)
  const NO_BADGE_COOLDOWN_MS = 90 * 1000;
  const STORAGE_NEXT = '__baiakIdleColetarRecompensaNextAt';

  class BaiakIdleColetarRecompensaModule {
    constructor() {
      this._running = false;
      this._busy = false;
    ...
    _log(msg, extra) {
      try {
        if (window.__BAIAKIDLE_DEBUG) {
          console.log('[BaiakIdle Coletar Rewards]', msg, extra || '');
        }
      } catch (_) {}
    }
    ...
    async _waitFor(fn, timeoutMs, intervalMs = 120) {
        ...
        window.postMessage(
          {
            ...
            payload: {
              botId: 'baiak_idle',
              botLabel: 'Baiak-Idle',
              moduleId: 'coletar_recompensa',
              moduleLabel: 'Daily Reward',
              status,
              remainingMs: 0,
    ...
    _startStatusTicker() {
      ...
      const tick = () => {
        ...
        if (this._busy) {
          this._emitStatus('claiming', this._lastAction || 'â¦');
          return;
        }
        const next = this._getNextAt();
        if (next && next > Date.now()) {
          this._emitStatus('waiting', this._formatRemain(next - Date.now()));
          return;
        }
        if (this._isRewardAvailable()) {
          this._emitStatus('ready', 'disponÃ­vel');
        } else {
          this._emitStatus('idle', 'aguardando badge');
        }
      };
      tick();
    ...
    _getNextAt() {
      try {
        const raw = sessionStorage.getItem(STORAGE_NEXT);
    ...
    _setNextAt(ts) {
      try {
        sessionStorage.setItem(STORAGE_NEXT, String(ts));
      } catch (_) {}
    }
    ...
    _dailyTab() {
      return (
        document.querySelector('#tab-daily') ||
        document.querySelector('.tab#tab-daily') ||
        document.querySelector('[id="tab-daily"]')
      );
    }

    _dailyBadge(tab) {
      const root = tab || this._dailyTab();
      if (!root) return null;
      return (
        root.querySelector('.daily-badge') ||
        root.querySelector('[class*="daily-badge"]')
      );
    }

    _textLooksAvailable(text) {
      const t = String(text || '')
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
      return (
        ...
        (t.includes('coleta') && t.includes('reward')) ||
        t.includes('para coletar')
      );
    }

    /** True quando a recompensa diÃ¡ria estÃ¡ liberada. */
    _isRewardAvailable() {
      // Modal jÃ¡ aberto com botÃ£o de claim
      const claim = this._claimBtn();
      if (claim && this._visible(claim) && !claim.disabled) return true;

      const tab = this._dailyTab();
      if (!tab) return false;

      const badge = this._dailyBadge(tab);
      if (badge && this._visible(badge)) return true;
      // badge pode ser sÃ³ um span sem tamanho; ainda conta se estÃ¡ no DOM
      if (badge) return true;

      const hints = [
        tab.getAttribute('title'),
        tab.getAttribute('data-tip'),
        tab.getAttribute('data-i18n-title'),
        badge?.getAttribute?.('title')
      ]
        .filter(Boolean)
        .join(' ');
      return this._textLooksAvailable(hints);
    }

    _claimBtn() {
      return (
        document.querySelector('#daily-claim-btn') ||
        document.querySelector('button.daily-claim-btn') ||
        document.querySelector('#daily-claim-slot button')
      );
    }

    _closeBtn() {
      return (
        document.querySelector('#daily-modal-close') ||
        document.querySelector('#daily-modal button.im-closebtn') ||
        document.querySelector('.daily-modal .im-closebtn')
      );
    }
    ...
    _humanClick(el, label) {
      ...
      try {
        if (typeof PointerEvent === 'function') {
          ...
          el.dispatchEvent(new PointerEvent('pointerup', { ...pOpts, buttons: 0, pressure: 0 }));
        }
      } catch (_) {}
    ...
    _clickSafe(el, label) {
      ...
      try {
        const S = this._sel();
        if (S && typeof S.isClickable === 'function' && !S.isClickable(el)) {
          // tab/badge Ã s vezes falham isClickable; tenta o mesmo
          if (!el.id || (el.id !== 'tab-daily' && el.id !== 'daily-claim-btn' && el.id !== 'daily-modal-close')) {
            return false;
          }
        }
      } catch (_) {}
    ...
    async _claimFlow() {
      this._lastAction = 'abrindo Dailyâ¦';
      const tab = this._dailyTab();
      if (!tab) {
        this._log('tab-daily ausente');
        return false;
      }

      // Se claim jÃ¡ visÃ­vel, pular abertura
      let claim = this._claimBtn();
      if (!(claim && this._visible(claim))) {
        this._clickSafe(tab, 'tab-daily');
        claim = await this._waitFor(() => {
          const b = this._claimBtn();
          return b && this._visible(b) && !b.disabled ? b : null;
        }, MODAL_WAIT_MS);
      }

      if (!claim) {
        this._log('claim btn nÃ£o apareceu');
        // fecha modal se abriu sem claim (jÃ¡ coletado)
        const closeOnly = this._closeBtn();
        if (closeOnly && this._visible(closeOnly)) {
          this._clickSafe(closeOnly, 'daily-modal-close empty');
        }
        return false;
      }

      this._lastAction = 'coletandoâ¦';
      this._clickSafe(claim, 'daily-claim-btn');
      await this._sleep(SETTLE_MS + 400);

      this._lastAction = 'fechandoâ¦';
      const close = await this._waitFor(() => {
        const b = this._closeBtn();
        return b && this._visible(b) ? b : null;
      }, 3000);
      if (close) {
        this._clickSafe(close, 'daily-modal-close');
    ...
    async _tick() {
      ...
      if (!this._isRewardAvailable()) {
        // sem badge: evita re-poll agressivo
        this._setNextAt(Date.now() + NO_BADGE_COOLDOWN_MS);
        return;
      }

      this._busy = true;
      try {
        const ok = await this._claimFlow();
        if (ok) {
          this._setNextAt(Date.now() + CLAIM_COOLDOWN_MS);
          this._emitStatus('claimed', 'ok');
          this._log('claim ok');
        } else {
          this._setNextAt(Date.now() + NO_BADGE_COOLDOWN_MS);
        }
    ...
    start() {
      ...
      this._observer = new MutationObserver(() => {
        // badge pode surgir sem passar pelo poll
        if (this._isRewardAvailable()) this._scheduleCheck();
      });
      ...
      if (root) {
        this._observer.observe(root, {
          ...
          subtree: true,
          attributes: true,
          attributeFilter: ['class', 'title', 'data-tip', 'style', 'hidden']
        });
      }
  ...
  window.BaiakIdleColetarRecompensaModule = BaiakIdleColetarRecompensaModule;

  try {
    const prev = window.__baiakIdleColetarRecompensa;
    const wasRunning = !!prev?.isRunning?.();
    try {
      prev?.stop?.();
    } catch (_) {}
    window.__baiakIdleColetarRecompensa = new BaiakIdleColetarRecompensaModule();
    if (wasRunning || window.__BAIAKIDLE_AUTO_START_COLETAR_RECOMPENSA__) {
      window.__baiakIdleColetarRecompensa.start();
    }
  } catch (err) {
    console.error('[BaiakIdle Coletar Rewards] bootstrap', err);
  }
})();
