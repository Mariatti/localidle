/* TB-WM u=179 e=a07d5b4cfa x=f2e5047b9289 t=1786720267 s=fe09be4d0b41554e */
(function(){try{window.__TIBIABOT_WM__={u:179,t:1786720267,x:"f2e5047b9289",s:"fe09be4d0b41554e"};}catch(e){}})();
...
// MÃ³dulo Baiak Idle (MAIN): Retornar Hunt
// 1) Se localizar Cidade (#wave-title) â estÃ¡ online â goToHunt()
// 2) Se NÃO estiver na cidade â procura popup de manutenÃ§Ã£o (#conn-retry)
// Bundle: seletores + teleporte + este mÃ³dulo
...
(function () {
    ...
    class BaiakIdleRetornarHuntModule {
        ...
        _emitStatus(status, remainingText) {
            // Overlay: sÃ³ o nome do mÃ³dulo (sem "reconectarâ¦", "cidade", etc.).
            const key = status + '|' + (this._running ? 1 : 0);
        ...
        _isMaintenanceVisible() {
            ...
            if (card) {
                const title = String(
                    ...
                    .trim()
                    .toUpperCase();
                if (title.includes('MANUTEN') || title.includes('MANUTENÃÃO') || title.includes('MANUTENCAO')) {
                    return true;
                }
            }
            // BotÃ£o presente jÃ¡ basta (tela de conexÃ£o/manutenÃ§Ã£o)
            return true;
        }
        ...
        async _clickRetry() {
            ...
            this._emitStatus('retry', 'reconectarâ¦');
            try {
                this._click(btn);
        ...
        async _tick() {
            ...
            try {
                // 1) Cidade = online â retornar para a hunt
                if (this._isInCity()) {
                ...
                // 2) Sem cidade â tenta popup de manutenÃ§Ã£o
