/* TB-WM u=179 e=a07d5b4cfa x=f2e5047b9289 t=1786720274 s=043c043867ece8da */
(function(){try{window.__TIBIABOT_WM__={u:179,t:1786720274,x:"f2e5047b9289",s:"043c043867ece8da"};}catch(e){}})();
...
// MÃ³dulo Baiak Idle (MAIN): MÃ©dia Gold (Market â HistÃ³rico)
// Abre Comercio â Market â HistÃ³rico â Gold, lÃª 3 pÃ¡ginas e calcula
// coins mÃ©dios por 100.000.000 gold (100kk).

(function () {
  const GOLD_UNIT = 100000000;
  const PAGES = 3;
  const STEP_TIMEOUT_MS = 20000;
  const PAGE_LOAD_MS = 12000;
  const STYLE_ID = 'tb-gold-media-styles';
  const MODAL_ID = 'tb-gold-media-modal';
  const PIN_CLASS = 'is-tb-forced-open';

  class BaiakIdleGoldMediaModule {
    constructor() {
      this._running = false;
      this._busy = false;
    }
    ...
    _log(msg, extra) {
      try {
        if (window.__BAIAKIDLE_DEBUG) {
          console.log('[BaiakIdle Gold Media]', msg, extra || '');
        }
      } catch (_) {}
    }
    ...
    _safeClick(el) {
      if (!el) return false;
      const S = this._sel();
      if (typeof S?.safeClick === 'function') return !!S.safeClick(el);
      try {
    ...
    async _waitFor(fn, timeoutMs, intervalMs) {
      const limit = Math.max(500, Number(timeoutMs) || STEP_TIMEOUT_MS);
      const step = Math.max(40, Number(intervalMs) || 120);
    ...
    _ensureStyles() {
      ...
      st.textContent =
        ...
        'background:rgba(4,8,14,.62);font-family:system-ui,-apple-system,Segoe UI,sans-serif;}' +
        '#' +
        MODAL_ID +
        ' .tb-gm-card{min-width:min(92vw,320px);max-width:92vw;background:#121a24;color:#e8eef6;' +
        'border:1px solid rgba(148,163,184,.28);border-radius:14px;padding:22px 20px 18px;box-shadow:0 18px 50px rgba(0,0,0,.45);' +
        'text-align:center;}' +
        '#' +
        MODAL_ID +
        ' .tb-gm-title{font-size:13px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:#94a3b8;margin:0 0 10px;}' +
        '#' +
        MODAL_ID +
        ' .tb-gm-body{font-size:16px;line-height:1.45;color:#f1f5f9;margin:0 0 6px;}' +
        '#' +
        MODAL_ID +
        ' .tb-gm-value{font-size:28px;font-weight:800;color:#fbbf24;margin:8px 0 4px;}' +
        '#' +
        MODAL_ID +
        ' .tb-gm-sub{font-size:12px;color:#94a3b8;margin:0 0 14px;line-height:1.4;}' +
        '#' +
        MODAL_ID +
        ' .tb-gm-err{color:#fca5a5;font-size:14px;margin:8px 0 14px;line-height:1.4;}' +
        '#' +
        MODAL_ID +
        ' .tb-gm-spin{width:28px;height:28px;margin:12px auto 4px;border:3px solid rgba(148,163,184,.25);' +
        'border-top-color:#fbbf24;border-radius:50%;animation:tb-gm-spin .7s linear infinite;}' +
        '#' +
        MODAL_ID +
        ' .tb-gm-close{appearance:none;border:0;border-radius:10px;padding:10px 18px;font-size:14px;font-weight:700;' +
        'cursor:pointer;background:#fbbf24;color:#1a1203;}' +
        '#' +
        MODAL_ID +
        ' .tb-gm-close:hover{filter:brightness(1.06);}' +
        '@keyframes tb-gm-spin{to{transform:rotate(360deg);}}' +
        '.tab-group.' +
        PIN_CLASS +
        ' [role="menu"],' +
        '.tab-group.' +
        ...
        '.tab-group.' +
        PIN_CLASS +
        ' #tab-auction{display:flex!important;visibility:visible!important;opacity:1!important;' +
        'pointer-events:auto!important;}';
      (document.head || document.documentElement).appendChild(st);
    }
    ...
    _showModal(state) {
      ...
      const mode = state && state.mode ? state.mode : 'loading';
      if (mode === 'loading') {
        root.innerHTML =
          '<div class="tb-gm-card">' +
          '<p class="tb-gm-title">MÃ©dia Gold</p>' +
          '<div class="tb-gm-spin" aria-hidden="true"></div>' +
          '<p class="tb-gm-body">Pesquisando...</p>' +
          '<p class="tb-gm-sub">' +
          String(state.detail || 'Abrindo Market e lendo o histÃ³rico de gold.') +
          '</p>' +
          '</div>';
        return;
      }
      if (mode === 'error') {
        root.innerHTML =
          ...
          '<p class="tb-gm-title">MÃ©dia Gold</p>' +
          '<p class="tb-gm-err"></p>' +
          '<button type="button" class="tb-gm-close" data-tb-gm-close="1">Fechar</button>' +
          '</div>';
        const err = root.querySelector('.tb-gm-err');
        if (err) err.textContent = String(state.message || 'Falha na anÃ¡lise.');
        root.querySelector('[data-tb-gm-close]')?.addEventListener('click', () => this._hideModal());
        return;
      }
      // result
      root.innerHTML =
        '<div class="tb-gm-card">' +
        '<p class="tb-gm-title">MÃ©dia Gold</p>' +
        '<p class="tb-gm-body">A cada 100.000.000 gold</p>' +
        '<p class="tb-gm-value"></p>' +
        '<p class="tb-gm-sub"></p>' +
        '<button type="button" class="tb-gm-close" data-tb-gm-close="1">Fechar</button>' +
        '</div>';
      const val = root.querySelector('.tb-gm-value');
      const sub = root.querySelector('.tb-gm-sub');
      if (val) {
        val.textContent =
          this._formatNum(state.coinsPer100m, 2) + ' coins';
      }
      if (sub) {
        sub.textContent =
          (state.count || 0) +
          ' venda(s) Â· ' +
          this._formatNum(state.totalGold, 0) +
          ' gold Â· ' +
          this._formatNum(state.totalCoins, 0) +
          ' coins Â· ' +
          (state.pages || 0) +
          ' pÃ¡g.';
      }
    ...
    _unpinComercio() {
      try {
        document.querySelectorAll('.' + PIN_CLASS).forEach((el) => {
          el.classList.remove(PIN_CLASS, 'open', 'hover', 'is-open');
        });
        const t = document.getElementById('tab-comercio');
        if (t) t.setAttribute('aria-expanded', 'false');
      } catch (_) {}
    }

    _pinComercioMenu() {
      const tab = document.getElementById('tab-comercio');
      if (!tab) return null;
      const group =
        tab.closest('.tab-group') ||
        tab.closest('[class*="tab-group"]') ||
      ...
      this._ensureStyles();
      if (group) {
        group.classList.add(PIN_CLASS, 'open', 'hover', 'is-open');
      }
      try {
        tab.setAttribute('aria-expanded', 'true');
      } catch (_) {}
      ...
      // forÃ§a nÃ³s ancestrais do Market visÃ­veis
      try {
        const market = document.getElementById('tab-auction');
        let p = market;
        while (p && p !== document.body) {
          try {
            ...
            if (Number(cs.opacity) === 0) p.style.opacity = '1';
    ...
    _findHistoricoBtn() {
      const prefer = document.querySelector('button.mk-navi[aria-label="Historico"]');
      if (prefer) return prefer;
      const list = document.querySelectorAll('button.mk-navi');
      for (let i = 0; i < list.length; i++) {
        const t = String(list[i].textContent || '')
          .replace(/\s+/g, ' ')
          .trim()
          .toLowerCase();
        if (t.includes('historico') || t.includes('histÃ³rico')) return list[i];
      }
      return null;
    }

    _findGoldFilterBtn() {
      const segs = document.querySelectorAll('.ac-seg button');
      for (let i = 0; i < segs.length; i++) {
        const t = String(segs[i].textContent || '')
          .trim()
          .toLowerCase();
        if (t === 'gold') return segs[i];
      }
      return null;
    }

    _readPageLabel() {
      const span = document.querySelector('.mk-pager span.muted, .mk-pager span');
    ...
    _findNextBtn() {
      const pager = document.querySelector('.mk-pager');
      if (!pager) return null;
      const buttons = pager.querySelectorAll('button');
      if (!buttons.length) return null;
      // Ãºltimo botÃ£o costuma ser "Proxima >"
      for (let i = buttons.length - 1; i >= 0; i--) {
        const b = buttons[i];
        const t = String(b.textContent || '')
          .toLowerCase()
          .replace(/\s+/g, ' ');
        if (t.includes('proxim') || t.includes('next') || t.includes('>')) return b;
      }
      return buttons[buttons.length - 1];
    }

    _parseCards() {
      const cards = document.querySelectorAll('.mk-grid .mk-card.ac-ended, .mk-grid .store-card.mk-card');
      /** @type {{ id: string, gold: number, coins: number }[]} */
      const rows = [];
      cards.forEach((card) => {
        const goldEl = card.querySelector('.mk-goldamt');
        const priceEl = card.querySelector('.mk-price.ac-sold, .mk-price');
        const idEl = card.querySelector('.mk-idtag');
        if (!goldEl || !priceEl) return;
        const gold = this._parseBrNumber(goldEl.textContent);
        const priceText = String(priceEl.textContent || '');
        // "Lance ganhador: 50 coins"
        let coins = null;
        const m = priceText.match(/([\d.]+)\s*coins?/i);
        if (m) coins = this._parseBrNumber(m[1]);
        else coins = this._parseBrNumber(priceText);
        if (gold == null || gold <= 0 || coins == null || coins < 0) return;
        rows.push({
          id: String(idEl?.textContent || '').trim(),
          gold,
          coins
        });
      });
    ...
    async _waitCardsStable() {
      await this._waitFor(() => {
        const cards = document.querySelectorAll('.mk-grid .mk-card .mk-goldamt');
        return cards.length > 0 ? cards : null;
      }, PAGE_LOAD_MS);
      await this._sleep(350);
      return this._parseCards();
    }

    async _goPage(fromLabel) {
      const next = this._findNextBtn();
      if (!next || next.disabled) {
        throw new Error('BotÃ£o PrÃ³xima pÃ¡gina indisponÃ­vel.');
      }
      this._safeClick(next);
      const ok = await this._waitFor(() => {
        const label = this._readPageLabel();
        if (!label) return null;
        if (fromLabel && label === fromLabel) return null;
        if (document.querySelectorAll('.mk-grid .mk-card .mk-goldamt').length < 1) return null;
        return label;
      }, PAGE_LOAD_MS);
      if (!ok) throw new Error('PÃ¡gina do Market nÃ£o carregou a tempo.');
      await this._sleep(280);
      return ok;
    }

    async _openMarketHistoricoGold() {
      this._showModal({ mode: 'loading', detail: 'Abrindo Comercioâ¦' });
      const comercio = await this._waitFor(
        () => document.getElementById('tab-comercio'),
        STEP_TIMEOUT_MS
      );
      if (!comercio) throw new Error('Aba Comercio nÃ£o encontrada.');

      this._pinComercioMenu();
      await this._sleep(250);
      this._pinComercioMenu();

      this._showModal({ mode: 'loading', detail: 'Abrindo Marketâ¦' });
      const market = await this._waitFor(() => {
        this._pinComercioMenu();
        return document.getElementById('tab-auction');
      }, STEP_TIMEOUT_MS);
      if (!market) throw new Error('Aba Market nÃ£o encontrada (menu Comercio).');
      this._safeClick(market);
      await this._sleep(400);

      this._showModal({ mode: 'loading', detail: 'Abrindo HistÃ³ricoâ¦' });
      const hist = await this._waitFor(() => this._findHistoricoBtn(), STEP_TIMEOUT_MS);
      if (!hist) throw new Error('BotÃ£o HistÃ³rico nÃ£o encontrado.');
      this._safeClick(hist);
      await this._sleep(450);

      this._showModal({ mode: 'loading', detail: 'Filtrando Goldâ¦' });
      const goldBtn = await this._waitFor(() => this._findGoldFilterBtn(), STEP_TIMEOUT_MS);
      if (!goldBtn) throw new Error('Filtro Gold nÃ£o encontrado.');
      if (!goldBtn.classList.contains('on')) {
        this._safeClick(goldBtn);
      }
    ...
    async _closeAuction() {
      try {
        const closeBtn =
          document.getElementById('auction-modal-close') ||
          document.querySelector('button.im-closebtn');
        if (closeBtn) this._safeClick(closeBtn);
      } catch (_) {}
      this._unpinComercio();
    }
    ...
    async analyze() {
      if (this._busy) {
        this._showModal({ mode: 'loading', detail: 'JÃ¡ hÃ¡ uma pesquisa em andamentoâ¦' });
      ...
      try {
        this._showModal({ mode: 'loading', detail: 'Pesquisando...' });
        await this._openMarketHistoricoGold();

        for (let page = 1; page <= PAGES; page++) {
          this._showModal({
            mode: 'loading',
            detail: 'Lendo pÃ¡gina ' + page + ' de ' + PAGES + 'â¦'
          });
          const before = this._readPageLabel();
          const rows = await this._waitCardsStable();
          rows.forEach((r) => all.push(r));
          pagesDone = page;
          this._log('pÃ¡gina ' + page, { n: rows.length, label: before });

          if (page < PAGES) {
            await this._goPage(before);
          }
        }

        await this._closeAuction();
        ...
        for (let i = 0; i < all.length; i++) {
          const r = all[i];
          const key = r.id || i + ':' + r.gold + ':' + r.coins;
        ...
        if (totalGold <= 0 || seen.size === 0) {
          throw new Error('Nenhuma venda de gold encontrada no histÃ³rico.');
        }

        const coinsPer100m = (totalCoins / totalGold) * GOLD_UNIT;
        const result = {
          success: true,
          coinsPer100m,
          totalGold,
        ...
        this._log('resultado', result);
        return result;
      } catch (err) {
        try {
          await this._closeAuction();
        } catch (_) {}
        const message = String(err && err.message ? err.message : err || 'Erro desconhecido');
        this._showModal({ mode: 'error', message });
        this._log('erro', message);
    ...
    /** InÃ­cio = roda a anÃ¡lise once (botÃ£o do painel). */
    start() {
      void this.analyze();
      return { success: true };
    }
    ...
    stop() {
      ...
      this._busy = false;
      this._unpinComercio();
      // nÃ£o remove modal de resultado automaticamente
      return { success: true };
    }

    isRunning() {
      return !!(this._running || this._busy);
    }
  }

  window.BaiakIdleGoldMediaModule = BaiakIdleGoldMediaModule;

  try {
    const prev = window.__baiakIdleGoldMedia;
    try {
      prev?.stop?.();
    } catch (_) {}
    window.__baiakIdleGoldMedia = new BaiakIdleGoldMediaModule();
    if (window.__BAIAKIDLE_AUTO_START_GOLD_MEDIA__) {
      window.__baiakIdleGoldMedia.start();
    }
  } catch (err) {
    console.error('[BaiakIdle Gold Media] Falha no bootstrap', err);
  }
})();
