...
// Toasts de boss (kill/death): MutationObserver só com AutoBoss/confront pendente (não no idle).
...
(function () {
  ...
  /** CSP da página bloqueia blob:/inline — background injeta via executeScript world:MAIN. */
  function pushMainVars(vars) {
    try {
      chrome.runtime.sendMessage({
        type: 'TIBIA_BOT_PUSH_MAIN_VARS',
  ...
  function pushSelectedHuntToPage(hunt) {
    pushMainVars({
      selectedHunt: hunt && typeof hunt === 'object' ? hunt : null
    });
  }
  ...
  function pushStaminaConfigToPage(cfg) {
    pushMainVars({
      staminaConfig:
        cfg && typeof cfg === 'object' ? cfg : { minPct: 15, maxPct: 30 }
    });
  }
  ...
  function pushAutoAnuncioConfigToPage(cfg) {
    ...
    pushMainVars({
      autoAnuncioConfig: { channel, text, intervalMin }
    });
  }
  ...
  function effectiveBossCooldownExpiresAt(row) {
    if (!row || typeof row !== 'object') return 0;
    const anchor =
      Number(row.finishedAt) || Number(row.startedAt) || Number(row.expiresAt) || 0;
    if (!anchor) return Number(row.expiresAt) || 0;
    const daily = bossDailyCooldownExpiresAt(anchor);
    const stored = Number(row.expiresAt) || 0;
  ...
  async function markBossFinished(outcome, bossName) {
    ...
    byId[targetId] = {
      ...
      expiresAt: prev.expiresAt || bossDailyCooldownExpiresAt(now),
      finished: true,
      finishedAt: now,
      outcome: kind,
      killed: kind === 'kill',
  ...
  /** MutationObserver de toasts de boss — só ligado com AutoBoss/confront ativo. */
  let bossOutcomeObserver = null;

  function shouldWatchBossOutcome(storageSlice) {
    const run = storageSlice?.[AUTOBOSS_RUN_KEY];
    if (run && run.running) return true;
    if (storageSlice?.baiakIdleAutoBossEnabled) return true;
    // Só com confront pendente (toast kill/death). Histórico byId sozinho não mantém o observer.
    const track = storageSlice?.[BOSS_TRACK_KEY];
    return !!(track && track.pendingId);
  }

  function stopBossOutcomeWatcher() {
    if (!bossOutcomeObserver) return;
    try {
      bossOutcomeObserver.disconnect();
    } catch (_) {}
    bossOutcomeObserver = null;
  }
  ...
  function startBossOutcomeWatcher() {
    ...
    bossOutcomeObserver = new MutationObserver((mutations) => {
    ...
    // Sem characterData: toasts costumam ser nós novos; characterData no document inteiro é caro.
    bossOutcomeObserver.observe(root, {
      childList: true,
      subtree: true
    });
  }

  async function syncBossOutcomeWatcher() {
    try {
      const data = await chrome.storage.local.get([
        BOSS_TRACK_KEY,
        AUTOBOSS_RUN_KEY,
        'baiakIdleAutoBossEnabled'
      ]);
      if (shouldWatchBossOutcome(data)) startBossOutcomeWatcher();
      else stopBossOutcomeWatcher();
    } catch (_) {
      stopBossOutcomeWatcher();
    }
  }
    ...
    window.addEventListener('message', (event) => {
      ...
      if (data.type === 'AUTOBOSS_BOSS_FINISHED') {
        ...
        if (outcome === 'kill' || outcome === 'death') {
          void markBossFinished(outcome, p.bossName || '').then(() => syncBossOutcomeWatcher());
          return;
        }
      ...
      if (data.type === 'AUTOBOSS_PROGRESS') {
        ...
        void chrome.storage.local.set({
          [AUTOBOSS_RUN_KEY]: {
            ...
            fightTimerRemainingMs: Number(p.fightTimerRemainingMs) || 0,
            fightTimerText: String(p.fightTimerText || ''),
            fightTimerEndsAt: Number(p.fightTimerEndsAt) || 0
          }
        });
        if (p.running) startBossOutcomeWatcher();
        else void syncBossOutcomeWatcher();
        return;
      }
  ...
  try {
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (
        changes[BOSS_TRACK_KEY] ||
        changes[AUTOBOSS_RUN_KEY] ||
        changes.baiakIdleAutoBossEnabled
      ) {
        void syncBossOutcomeWatcher();
      }
    });
