...
(function () {
  ...
  // Observer só no aside (não no document inteiro) — evita custo contínuo no DOM do jogo.
  ...
  let watchingAside = null;

  function watchAside(aside) {
    if (!aside || watchingAside === aside) return;
    try {
      observer.disconnect();
    } catch (_) {}
    watchingAside = aside;
    // Só filhos diretos do aside (reposicionamento do panel), sem subtree profunda.
    observer.observe(aside, { childList: true, subtree: false });
  }
  ...
  function watch() {
    ...
    // Aside ainda não existe: tenta de novo sem MO no document.
  ...
  // Backup leve se o jogo recriar o aside.
  setInterval(() => {
    const aside = findAsideCol();
    if (!aside) return;
    if (watchingAside !== aside) watchAside(aside);
    ensurePanel();
  }, 5000);
