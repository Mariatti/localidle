...
(function () {
  ...
  const AUTOBOSS_AUTO_CYCLE_KEY = 'baiakIdleAutoBossAutoCycle';
  const AUTOBOSS_CYCLE_STARTED_AT_KEY = 'baiakIdleAutoBossCycleStartedAt';
  const AUTOBOSS_CYCLE_HOUR = 0;
  const AUTOBOSS_CYCLE_MINUTE = 5;
  const AUTOBOSS_CYCLE_LABEL = 'AutBoss 00:05';
  ...
  async function loadAutoBossCycle() {
    try {
      const data = await chrome.storage.local.get([
        AUTOBOSS_AUTO_CYCLE_KEY,
        AUTOBOSS_CYCLE_STARTED_AT_KEY
      ]);
      autoBossAutoCycle = !!data[AUTOBOSS_AUTO_CYCLE_KEY];
      autoBossCycleStartedAt = Number(data[AUTOBOSS_CYCLE_STARTED_AT_KEY]) || 0;
    } catch (_) {
  ...
  function formatAutoBossCycleRemaining() {
    ...
    if (dueAt <= Date.now()) return 'reinício pendente';
  ...
  function bindStop(el) {
    ...
    el.addEventListener('click', (event) => {
      ...
      try {
        window.postMessage(
          { source: 'TIBIA_BOT_CONTENT', type: 'AUTOBOSS_REQUEST_STOP' },
          '*'
        );
      } catch (_) {}
      // Também para o módulo via runtime (painel / SW).
      try {
        chrome.runtime.sendMessage({ type: 'BAIAKIDLE_STOP_AUTOBOSS' });
      } catch (_) {}
      autoBossRun = null;
      render();
    });
  ...
  function ensureOverlay() {
    ...
    if (el) {
      // Hot-reload / overlay antigo: garante altura máxima + scroll no body.
      ...
      const bodyExisting = el.querySelector('[data-role="body"]');
      if (bodyExisting) {
        bodyExisting.style.flex = '1 1 auto';
        bodyExisting.style.minHeight = '0';
        bodyExisting.style.overflowX = 'hidden';
        bodyExisting.style.overflowY = 'auto';
        bodyExisting.style.overscrollBehavior = 'contain';
      }
      const handleExisting = el.querySelector('[data-role="drag-handle"]');
      if (handleExisting) handleExisting.style.flex = '0 0 auto';
      bindDrag(el);
      bindStop(el);
    ...
    el.innerHTML = [
      '<div data-role="drag-handle" title="Arraste para mover" style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:8px;cursor:grab;flex:0 0 auto;">',
      '<div data-role="brand">TibiaBot.Online</div>',
      '<div style="font-size:11px;color:#93a4b8;letter-spacing:.12em;">⋮⋮</div>',
      '</div>',
      '<div data-role="body" style="flex:1 1 auto;min-height:0;overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;"></div>'
    ].join('');
  ...
  /** Tempo da Session do jogo (#an-session), ex.: "01:00:12". */
  ...
  function formatFightTimerMs(ms) {
  ...
  /** Countdown local do timer de luta (decai no overlay a cada 1s). */
  function getFightTimerDisplayText() {
    const run = autoBossRun;
    if (!run || !run.running) return '';
    const endsAt = Number(run.fightTimerEndsAt) || 0;
    if (endsAt > 0) {
      const rem = endsAt - Date.now();
      if (rem <= 0) return '00:00';
      return formatFightTimerMs(rem);
    }
    return String(run.fightTimerText || '');
  }

  function syncFightTimerLabel() {
    const root = document.getElementById(OVERLAY_ID);
    if (!root || root.style.display === 'none') return;
    const slot = root.querySelector('[data-role="fight-timer"]');
    if (!slot) return;
    const text = getFightTimerDisplayText();
    if (!text) {
      slot.textContent = '';
  ...
  function syncSessionLabel() {
    const root = document.getElementById(OVERLAY_ID);
    if (!root || root.style.display === 'none') return;
    const slot = root.querySelector('[data-role="session"]');
    if (!slot) return;
    const session = readGameSessionText();
    if (slot.textContent !== session) slot.textContent = session;
    slot.style.display = session ? '' : 'none';
  }

  function statusSuffix(moduleLabel, botId) {
    if (moduleLabel === AUTOBOSS_CYCLE_LABEL) {
      const text = formatAutoBossCycleRemaining();
      if (!text) return '';
      return `<span style="margin-left:auto;color:#3dba7a;font-variant-numeric:tabular-nums;">${escapeHtml(text)}</span>`;
    }
  ...
  function renderAutoBossHtml() {
    ...
    return (
      ...
      (() => {
        const t = getFightTimerDisplayText();
        return (
          `<span data-role="fight-timer" style="font-size:12px;font-weight:700;color:#fbbf24;font-variant-numeric:tabular-nums;${
            t ? '' : 'display:none;'
          }" title="Tempo restante no boss">${escapeHtml(t)}</span>`
        );
      })() +
      `<span style="font-size:11px;color:#93a4b8;">${done}/${total}</span>` +
      `<button type="button" data-ab-stop style="` +
  ...
  function getOverlayBots() {
    /** @type {Record<string, { botLabel: string, modules: string[] }>} */
    const bots = {};
    const src = modulesPayload?.bots || {};
    for (const [botId, bot] of Object.entries(src)) {
      bots[botId] = {
        botLabel: bot?.botLabel || botId,
        modules: Array.isArray(bot?.modules) ? bot.modules.slice() : []
      };
    }
    if (autoBossAutoCycle) {
      if (!bots.baiak_idle) {
        bots.baiak_idle = { botLabel: 'Baiak-Idle', modules: [] };
      }
      if (!bots.baiak_idle.modules.includes(AUTOBOSS_CYCLE_LABEL)) {
        bots.baiak_idle.modules.push(AUTOBOSS_CYCLE_LABEL);
      }
    }
    return bots;
  }
  ...
  function render() {
    ...
    const bots = getOverlayBots();
    const botIds = Object.keys(bots);
    const hasModules = botIds.some((id) => (bots[id].modules || []).length > 0);
    ...
    if (hasModules) {
      html = botIds
        .map((botId) => {
          ...
          const session =
            botId === 'baiak_idle' ? readGameSessionText() : '';
          const titleRow =
            `<div style="display:flex;align-items:baseline;justify-content:space-between;gap:8px;margin-bottom:2px;">` +
            `<div style="font-size:13px;font-weight:700;">${escapeHtml(bot.botLabel || botId)}</div>` +
            (botId === 'baiak_idle'
              ? `<div data-role="session" style="font-size:10px;font-weight:600;color:#93a4b8;font-variant-numeric:tabular-nums;letter-spacing:0.02em;${
                  session ? '' : 'display:none;'
                }">${escapeHtml(session)}</div>`
              : '') +
            `</div>`;
    ...
    body.innerHTML = html;
    el.style.maxWidth = hasAutoBoss ? '360px' : '320px';
    el.style.maxHeight = 'calc(100vh - 32px)';
    el.style.display = 'flex';
    el.style.flexDirection = 'column';
    el.style.overflow = 'hidden';
  ...
  function applyAutoBossRun(raw) {
    ...
    autoBossRun = {
      ...
      fightTimerRemainingMs: Number(raw.fightTimerRemainingMs) || 0,
      fightTimerText: String(raw.fightTimerText || ''),
      fightTimerEndsAt: (() => {
        const ends = Number(raw.fightTimerEndsAt) || 0;
        if (ends > 0) return ends;
        const rem = Number(raw.fightTimerRemainingMs) || 0;
        // compat: só remaining no payload → âncora local para countdown
        return rem > 0 ? Date.now() + rem : 0;
      })()
    };
    render();
  }
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (
      changes[AUTOBOSS_AUTO_CYCLE_KEY] ||
      changes[AUTOBOSS_CYCLE_STARTED_AT_KEY]
    ) {
      if (changes[AUTOBOSS_AUTO_CYCLE_KEY]) {
        autoBossAutoCycle = !!changes[AUTOBOSS_AUTO_CYCLE_KEY].newValue;
      }
      if (changes[AUTOBOSS_CYCLE_STARTED_AT_KEY]) {
        autoBossCycleStartedAt =
          Number(changes[AUTOBOSS_CYCLE_STARTED_AT_KEY].newValue) || 0;
      }
      render();
    }
  });
